...
const handleDeviceStatusEvents = () => {
    appClient.on("deviceStatus", function(deviceType, deviceId, payload, topic) {
        ...
        console.log("Status device ::" + deviceType + ":" + deviceId + "với payload:" + payload);
...
const processAddStatus = () => {
    jobs.process("status_device", 10, (job, done) => {
        ...
        DeviceDetail.findOneAndUpdate({ deviceId: deviceId }, { state: data }).then((result) => {
            console.log(result);
...
const getActiveDevices = (data) => {
    ...
    return new Promise((resolve, reject) => {
        appClient.getActiveDevices(startTime, endTime, true).then(function onSuccess(response) {
